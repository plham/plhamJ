 ...
 * <li>While it is possible for an {@link Agent} to access instances of {@link Market}, you should not call method
 * {@link Market#handleOrders(List)} as part of method {@link #submitOrders(List)},
 * <li>Call {@link #setMarketAccessible(Long)} to tell the agent to enter the specified market that market.
 * <li>Without calling <code>setMarketAccessible(Market)</code>, <code>getAssetVolume(Market)</code> will raise an
 * error.
 * <li>Use member {@link #random} to generate the random numbers needed for your agent. Do not use any other
 * {@link Random} object or the result of the simulation would not be reproducible with an initial seed
 * <li>implement a <code>public static void register(SimulatorFactory factory)</code> method in your child class which
 * registers a factory method for your class into the provided factory. Refer to
 * {@link FCNAgent#register(SimulatorFactory)} for an implementation example.
 * </ul>
 */
public abstract class Agent implements Serializable {
    ...
    /**
     * Return the {@link Random} object which is used by this object when some random number needs to be used.
     *
    ...
    /**
     * Sets the assets of this agent into the specified market to 0, also granting it access for trading. This method is
     * equivalent to calling method {@link #setMarketAccessible(Long)} with {@link Market#id} as parameter.
     *
