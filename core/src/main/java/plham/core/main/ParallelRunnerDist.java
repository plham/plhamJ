...
public final class ParallelRunnerDist extends Runner {
        ...
        for(Market m: markets) {
            try {
                if (t-3 < m.getTime() && m.getTime() < t+3) {
                    System.out.println(tag + ":" + bh.isMaster() + ":" + m.id + ":" + m.getTime() + ":" + m.getTradeVolume(t));
                }
    ...
    void updateMarketMisc(Session s, Fundamentals fundamentals) {
        ...
        //        long t1 = System.nanoTime();
        if (s.withPrint) {
            outputPrint(true, output, out, s, SimulationStage.WITH_PRINT_DURING_SESSION, bh.markets, bh.allAgents, sim.sessionEvents);
        }
        //        long t2 = System.nanoTime();
    ...
    public void run(final long seed, boolean pipeline) {
        ...
        try {
            ...
            //            final DistChunkedList<Agent> arbitrageurs = new DistCol<>(); // FIXME UNUSED!! Either arbitrageurs have a specific "schedule" or this should disappear
