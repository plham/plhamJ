...
//            "src/test/resources/BlackScholes/expected_shutdown_output.txt", "src/test/resources/BlackScholes/bs-warmup.json"}
        ...
        // For each problem we check with 3 lifeline strategies
        Collection<Object[]> toReturn = new ArrayList<>(parameters.size() * 3);
        parameters.forEach(a -> {
            Object[] noLifeline = Arrays.copyOf(a, a.length + 1);
            Object[] plhamLifeline = Arrays.copyOf(a, a.length + 1);
            Object[] hypercubeLifeline = Arrays.copyOf(a, a.length + 1);

            noLifeline[a.length] = NoLifeline.class.getCanonicalName();
            plhamLifeline[a.length] = PlhamLifeline.class.getCanonicalName();
            hypercubeLifeline[a.length] = Hypercube.class.getCanonicalName();

            // Modify the name
            noLifeline[0] = noLifeline[0] + " - No Lifeline";
            plhamLifeline[0] = plhamLifeline[0] + " - Plham Lifeline";
            hypercubeLifeline[0] = hypercubeLifeline[0] + " - Hypercube";

            toReturn.add(noLifeline);
            toReturn.add(plhamLifeline);
            toReturn.add(hypercubeLifeline);
        });
    ...
    public IT_GlbRunner(String name, Class<?> mainClass, String jsonFile, String seed, String expectedFileOutput, String warmupJson, String lifelineClass) {
