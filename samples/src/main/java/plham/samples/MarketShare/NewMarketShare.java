...
/**
 * Reference: Kusada, Mizuta, Hayakawa, Izumi, Yoshimura (2014) Analysis of the market makers spread's impact to markets
 * volume shares using an artificial market (in Japanese).
 */
public class NewMarketShare extends SimulationOutput {
    ...
    public void marketOutput(OutputCollector out, SimulationStage stage, Market market) {
        ...
        case WITH_PRINT_DURING_SESSION:
            ...
            out.log(market.name, String.format("%s %s %s %s %s %s  ", t, market.id, market.name, market.getPrice(),
                    market.getFundamentalPrice(), market.getTradeVolume()));
            break;
