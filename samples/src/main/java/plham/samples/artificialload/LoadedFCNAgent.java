...
public class LoadedFCNAgent extends plham.core.Agent {
    ...
    public static void register(SimulatorFactory sim) {
        final String className = "plham.samples.artificialload.LoadedFCNAgent";
        sim.addAgentInitializer(className, new AgentInitializer() {
            ...
            public Agent initialize(long id, String name, Random random, JSON.Value json) {
                return new LoadedFCNAgent(id, name, random).setup(json, sim);
            }
        });
    }
    ...
    /**
     * Maximum number of hash computation (artificial load) to compute when
     * submitting orders
     */
    private int maxHashComputation;

    /**
     * Minimum number of hash computation (artificial load) to compute when
     * submitting orders
     */
    private int minHashComputation;

    public LoadedFCNAgent(long id, String name, Random random) {
        super(id, name, random);
    }

    @Override
    public LoadedFCNAgent setup(JSON.Value json, SimulatorFactory sim) {
        final JSONRandom jsonRandom = new JSONRandom(random);
        ...
        if ("fixed".equals(json.getOrElse("marginType", "fixed").toString())) {
            marginType = LoadedFCNAgent.MARGIN_FIXED;
        } else {
            marginType = LoadedFCNAgent.MARGIN_NORMAL;
        }
        ...
        maxHashComputation = Integer.parseInt(json.getOrElse("maxload", "50").toString());
        minHashComputation = Integer.parseInt(json.getOrElse("minload", "0").toString());
        return this;
    }
    ...
    public List<Order> submitOrders(List<Market> markets) {
        /*
         * First, compute a random number of hashes
         */
        ...
        try {
            ...
            for (int toCompute = minHashComputation
                    + random.nextInt(maxHashComputation - minHashComputation); 0 < toCompute; toCompute--) {
                md.digest(hash, 0, 20);
            }
        ...
        /*
         * Then proceed with normal order decision
         */
    ...
    /*
     * public String submitOrdersInfo(Market market) { StringBuffer buf = new
     * StringBuffer(); buf.append("M:"+market.id + ":"+market.getClass()); if
     * (!this.isMarketAccessible(market)) { return "!isAccessible"; }
     * buf.append(", Rand:"+random.toString()); double fundamentalScale = 1.0 /
     * Math.max(meanReversionTime, 1); buf.append(", fundS:"+fundamentalScale);
     * buf.append(", fundP:"+market.getFundamentalPrice()); double
     * fundamentalLogReturn = fundamentalScale *
     * Math.log(market.getFundamentalPrice() / market.getPrice());
     * buf.append(", fundR:"+fundamentalLogReturn); double chartScale = 1.0 /
     * Math.max(timeWindowSize, 1); buf.append(", chartS:"+chartScale);
     * buf.append(", price:" + market.getPrice()); return buf.toString(); }
     */
    public List<Order> submitOrders(Market market) {
        ...
        final RandomHelper helper = new RandomHelper(random);

        final long t = market.getTime();

        final long timeWindowSize = Math.min(t, this.timeWindowSize);
        ...
        final double fundamentalScale = 1.0 / Math.max(meanReversionTime, 1);
        final double fundamentalLogReturn = fundamentalScale
                * Math.log(market.getFundamentalPrice() / market.getPrice());
        assert isFinite(fundamentalLogReturn) : "isFinite(fundamentalLogReturn)";

        final double chartScale = 1.0 / Math.max(timeWindowSize, 1);
        final double chartMeanLogReturn = chartScale
                * Math.log(market.getPrice() / market.getPrice(t - timeWindowSize));
        assert isFinite(chartMeanLogReturn) : "isFinite(chartMeanLogReturn)";

        final double noiseLogReturn = 0.0 + noiseScale * helper.nextGaussian();
        assert isFinite(noiseLogReturn) : "isFinite(noiseLogReturn)";

        final double expectedLogReturn = (1.0 / (fundamentalWeight + chartWeight + noiseWeight))
        ...
        final double expectedFuturePrice = market.getPrice() * Math.exp(expectedLogReturn * timeWindowSize);
        ...
        if (marginType == MARGIN_NORMAL) {
            ...
            final double orderPrice = expectedFuturePrice + helper.nextGaussian() * orderMargin;
            final long orderVolume = 1;
            assert orderPrice >= 0.0 : "orderPrice >= 0.0" + orderPrice;
            assert orderVolume >= 0 : "orderVolume >= 0" + orderVolume;
